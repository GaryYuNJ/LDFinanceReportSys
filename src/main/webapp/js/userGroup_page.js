	...
	var getUserURL = rootUri + "/userGroup/showUserGroupList.json";
	var pageNumber = 1;
	$('#userGroupTableId').bootstrapTable({
	 ...
	 function createUserGroup(){
		...
		$.ajax( {  
		    url:rootUri + "/userGroup/createNewGroup.json",
	  ...
	  //删除userGroup
		 function deleteUserGroupById(customerGroupId){
			...
			$.ajax( {  
			    url:rootUri + "/userGroup/deleteUserGroupById.json",
	  ...
	  var initUserGroupDetailForm = function (userGroupId) {
		  $.ajax( {  
			    url:rootUri + "/userGroup/showUserGroupDetail.json",
	...
	//更新 userGroup
	 $('#updateUserGroup_Button').click(function() {
		 ...
		 $.ajax({
		    url:rootUri + "/userGroup/updateUserGroup.json",
	...
	//关联用户组查询用户标识
	 ...
	 var userListTableInit = function () {
		...
		$('#userListTableId').bootstrapTable({
			method: 'get',
		    url: rootUri + "/user/showUserListWithGroupId.json", 
		    dataType: "json",
	...
	function resourceTableInit() {
		$('#resourceTableId').bootstrapTable({
			method: 'get',
		    url: rootUri + "/manage/resSearchWithCusGroupId.json", 
		    dataType: "json",
	...
	//更新用户组权限
	 $('#saveButton_addPer').click(function() {
        ...
        var endDateStr =$("#endDate_addPer").val();
 	    $.ajax({
 		    url:rootUri + "/userGroup/authCusGrpResPermission.json",
   ...
   function removePermission(resourceId,buttonObj){
	   ...
	   $.ajax({
		    url:rootUri + "/userGroup/disableCusGrpResPermission.json",
...
//用户组与资源组
	function resourceGroupTableInit() {
		var getgroupURL = rootUri + "/manage/resGroupSearchWithCusGrpId.json";
		var pageNumber = 1;
   ...
   function addResGroupPermPreProcess(resGrpId, resGrpName, startDate, endDate){
	...
	$('#saveButton_addGrpPer').click(function() {
      ...
      var endDateStr =$("#endDate_addGrpPer").val();
	    $.ajax({
		    url:rootUri + "/userGroup/authResGrpPermission.json",
	...
	//禁用资源
	   function removeResGroupPermission(resGroupId,buttonObj){
		   ...
		   $.ajax({
			    url:rootUri + "/manage/disableResGroupPermission.json",
